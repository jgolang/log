...
// Bits are order together to control what's printed.
...
// Production doc
func (f Formatter) Production(buf *[]byte, t time.Time, file string, line int, function string, p core.Priority, s string, stack [][]byte) {
...
func (f Formatter) Development(buf *[]byte, t time.Time, file string, line int, function string, p core.Priority, s string, stack [][]byte) {
...
func (f Formatter) jsonFormat(buf *[]byte, t time.Time, file string, line int, function string, p core.Priority, s string, stack [][]byte) {
	*buf = append(*buf, '{')
	*buf = append(*buf, "\"level\":\""...)
	*buf = append(*buf, core.GetTypeMsg(true, p)...)
	*buf = append(*buf, "\",\"ts\":\""...)
	ts := t.Unix()
	itoa(buf, ts, 10)
...
func (f Formatter) terminalFormat(buf *[]byte, t time.Time, file string, line int, function string, p core.Priority, s string, stack [][]byte) {
	...
	if f.devFlag&(Llevel) != 0 {
		*buf = append(*buf, core.GetTypeMsg(false, p)...)
		*buf = append(*buf, '\t')
	}
